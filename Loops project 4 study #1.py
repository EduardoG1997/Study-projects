...
# the operation of the business for the rest of the month. THIS IS FOR PRACTICE LOOPS 
...
# You can get the number of prices by using the len() function. Print the value of average_price so the output looks like:
...
# Use a list comprehension to make a list called new_prices, which has each element in prices minus 5. / Print new_prices.
...
# Create a variable called total_revenue and set it to 0.  Use a for loop to create a variable i that goes from 0 to len(hairstyles) 
# Add the product of prices[i] (the price of the haircut at position i) and last_week[i] (the number of people who got 
# the haircut at position i) to total_revenue at each step.
#After your loop, print the value of total_revenue, so the output looks like: Total Revenue: <total_revenue
...
average_dialy_revenue = total_revenue / 7 
